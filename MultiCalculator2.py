# Hika's Multi Purpose Python Project
...
def floatSum(a, b):
    f = a**3 + b**3 + a*b
    print("Float value of equation is: ", f)

def rectanglePerimeterSize(a, b):
    p = 2*(a + b)
    s = a*b
    print("The perimeter of your rectangle is: ", p)
    print("The size of your rectangle is: ", s)

def Velocity(h):
    v = (2*9.8*h)**1/2
    print("The velocity of this freefall motion is: ", v)

def coordinateDistance(a, b):
    d = ((a - 0)**2 + (b - 0)**2)**1/2
    print("The distance from (0, 0) to specified coordinate is: ", d)

def triangleSize(a, b, c):
    half_p = (a+b+c)/2
    s = (half_p*(half_p-a)*(half_p-b)*(half_p-c))**1/2
    print("The size of the specified triangle is: ", s)

# submodules GUI
def UI(a):
    if (a == 1):
        print("1: Calculate integer value of f(x) = x^10 + x^5 + 1")
        x = int(input("Enter integer value of x: "))
        intFunctionSum(x)
    elif (a == 2):
